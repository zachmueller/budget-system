...
IF OBJECT_ID('dbo.log_changes_workbooks', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbooks
GO


CREATE TRIGGER dbo.log_changes_workbooks
	ON BudgetDB.dbo.workbooks
...
SELECT @@SPID, @ci, workbook_id, 1, SYSTEM_USER
	,'workbooks', GETDATE(), APP_NAME(), action_taken
FROM (
