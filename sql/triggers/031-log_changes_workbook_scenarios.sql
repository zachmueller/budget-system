...
IF OBJECT_ID('dbo.log_changes_workbook_scenarios', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_scenarios
GO


CREATE TRIGGER dbo.log_changes_workbook_scenarios
	ON BudgetDB.dbo.workbook_scenarios
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_scenarios', GETDATE(), APP_NAME(), action_taken
FROM (
