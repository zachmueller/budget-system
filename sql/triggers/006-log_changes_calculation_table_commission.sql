...
IF OBJECT_ID('dbo.log_changes_calculation_table_commission', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_commission
GO


CREATE TRIGGER dbo.log_changes_calculation_table_commission
	ON BudgetDB.dbo.calculation_table_commission
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_commission', APP_NAME(), action_taken
FROM (
