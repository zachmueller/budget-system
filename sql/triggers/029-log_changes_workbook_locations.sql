...
IF OBJECT_ID('dbo.log_changes_workbook_locations', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_locations
GO


CREATE TRIGGER dbo.log_changes_workbook_locations
	ON BudgetDB.dbo.workbook_locations
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_locations', GETDATE(), APP_NAME(), action_taken
FROM (
