...
IF OBJECT_ID('dbo.log_changes_calculation_table_per_headcount_assumptions', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_per_headcount_assumptions
GO


CREATE TRIGGER dbo.log_changes_calculation_table_per_headcount_assumptions
	ON BudgetDB.dbo.calculation_table_per_headcount_assumptions
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_per_headcount_assumptions', APP_NAME(), action_taken
FROM (
