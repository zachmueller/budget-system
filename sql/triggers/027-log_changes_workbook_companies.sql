...
IF OBJECT_ID('dbo.log_changes_workbook_companies', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_companies
GO


CREATE TRIGGER dbo.log_changes_workbook_companies
	ON BudgetDB.dbo.workbook_companies
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_companies', GETDATE(), APP_NAME(), action_taken
FROM (
