...
IF OBJECT_ID('dbo.log_changes_calculation_table_base', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_base
GO


CREATE TRIGGER dbo.log_changes_calculation_table_base
	ON BudgetDB.dbo.calculation_table_base
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_base', APP_NAME(), action_taken
FROM (
