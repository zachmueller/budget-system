...
IF OBJECT_ID('dbo.log_changes_calculation_table_bonus', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_bonus
GO


CREATE TRIGGER dbo.log_changes_calculation_table_bonus
	ON BudgetDB.dbo.calculation_table_bonus
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_bonus', APP_NAME(), action_taken
FROM (
