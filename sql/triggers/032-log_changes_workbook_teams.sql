...
IF OBJECT_ID('dbo.log_changes_workbook_teams', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_teams
GO


CREATE TRIGGER dbo.log_changes_workbook_teams
	ON BudgetDB.dbo.workbook_teams
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_teams', GETDATE(), APP_NAME(), action_taken
FROM (
