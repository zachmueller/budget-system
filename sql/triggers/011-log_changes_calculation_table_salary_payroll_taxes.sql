...
IF OBJECT_ID('dbo.log_changes_calculation_table_salary_payroll_taxes', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_salary_payroll_taxes
GO


CREATE TRIGGER dbo.log_changes_calculation_table_salary_payroll_taxes
	ON BudgetDB.dbo.calculation_table_salary_payroll_taxes
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_salary_payroll_taxes', APP_NAME(), action_taken
FROM (
	...
	AND ISNULL(d.bu_number,'NULL BU')=ISNULL(i.bu_number,'NULL BU')
