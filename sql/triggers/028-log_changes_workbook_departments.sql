...
IF OBJECT_ID('dbo.log_changes_workbook_departments', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_departments
GO


CREATE TRIGGER dbo.log_changes_workbook_departments
	ON BudgetDB.dbo.workbook_departments
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_departments', GETDATE(), APP_NAME(), action_taken
FROM (
