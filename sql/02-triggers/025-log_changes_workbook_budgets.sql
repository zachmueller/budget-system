...
IF OBJECT_ID('dbo.log_changes_workbook_budgets', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_budgets
GO


CREATE TRIGGER dbo.log_changes_workbook_budgets
	ON BudgetDB.dbo.workbook_budgets
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_budgets', GETDATE(), APP_NAME(), action_taken
FROM (
