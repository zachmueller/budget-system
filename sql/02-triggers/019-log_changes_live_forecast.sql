...
IF OBJECT_ID('dbo.log_changes_live_forecast', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_live_forecast
GO


CREATE TRIGGER dbo.log_changes_live_forecast
	ON BudgetDB.dbo.live_forecast
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'live_forecast', GETDATE(), APP_NAME(), action_taken
FROM (
...
GROUP BY a.workbook_id, a.action_taken
