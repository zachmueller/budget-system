...
IF OBJECT_ID('dbo.log_changes_calculation_table_cap_rates', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_calculation_table_cap_rates
GO


CREATE TRIGGER dbo.log_changes_calculation_table_cap_rates
	ON BudgetDB.dbo.calculation_table_cap_rates
...
SELECT @@SPID, @ci, COUNT(*), SYSTEM_USER
	,GETDATE(), 'calculation_table_cap_rates', APP_NAME(), action_taken
FROM (
	--	select items either inserted or updated
	SELECT CASE WHEN d.id IS NULL THEN 'INSERT'
