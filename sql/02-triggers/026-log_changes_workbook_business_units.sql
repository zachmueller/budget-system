...
IF OBJECT_ID('dbo.log_changes_workbook_business_units', 'TR') IS NOT NULL
	DROP TRIGGER dbo.log_changes_workbook_business_units
GO


CREATE TRIGGER dbo.log_changes_workbook_business_units
	ON BudgetDB.dbo.workbook_business_units
...
SELECT @@SPID, @ci, workbook_id, COUNT(*), SYSTEM_USER
	,'workbook_business_units', GETDATE(), APP_NAME(), action_taken
FROM (
