...
IF OBJECT_ID('dbo.assumptions_bonus', 'P') IS NOT NULL
	DROP PROCEDURE dbo.assumptions_bonus
GO


CREATE PROCEDURE dbo.assumptions_bonus
	@wbID INT = 0
AS
/*
summary:	>
...
BEGIN	--	select out NULL values, though keep the same structure to not interfere with the Excel layout
	SELECT NULL [company_name], NULL [bu_name],NULL [dept_name],NULL [team_consolidation],NULL [location_name]
		,NULL [job_title],NULL [exp_item], NULL [avg_bonus]
END

ELSE 

BEGIN	--	select out relevant data
	...
	UNION	--	also always include dummy job title assumptions
	SELECT cp.company_name,bu.bu_name,dp.dept_name,ISNULL(tm.team_consolidation,tm.team_name) [team_consolidation]
		,lc.location_name,jt.job_title,pl.pl_item + ' - ' + pl.category_code [exp_item]
		,SUM(ft_pt_count*bonus_percent)/SUM(ft_pt_count) [avg_bonus]
	FROM BudgetDB.dbo.calculation_table_bonus a
