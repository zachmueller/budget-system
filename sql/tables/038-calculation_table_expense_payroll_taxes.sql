...
IF OBJECT_ID('dbo.calculation_table_expense_payroll_taxes', 'U') IS NOT NULL
	DROP TABLE dbo.calculation_table_expense_payroll_taxes
GO


CREATE TABLE dbo.calculation_table_expense_payroll_taxes (
	scenario_id INT NOT NULL
		CONSTRAINT fk_calculation_table_expense_payroll_taxes_scenario_id FOREIGN KEY
		REFERENCES dbo.scenarios (scenario_id),
	company_number NCHAR(3) NOT NULL
		CONSTRAINT fk_calculation_table_expense_payroll_taxes_company_number FOREIGN KEY
		REFERENCES dbo.companies (company_number),
	hfm_expense_code NVARCHAR(100) NOT NULL
		CONSTRAINT fk_calculation_table_expense_payroll_taxes_hfm_expense_code FOREIGN KEY
		REFERENCES dbo.pl_items (hfm_account_code),
	hfm_account_code NVARCHAR(100) NOT NULL
		CONSTRAINT fk_calculation_table_expense_payroll_taxes_hfm_account_code FOREIGN KEY
		REFERENCES dbo.pl_items (hfm_account_code),
	...
	CONSTRAINT pk_calculation_table_expense_payroll_taxes
		PRIMARY KEY CLUSTERED (scenario_id, company_number,
		hfm_expense_code, hfm_account_code)
)
GO
...
summary:	>
			...
			assumptions for payroll tax rates for expenses,
			generally employee benefit expenses (in the UK).
Revisions:
- version 1:
...
'
	...
	,@level0name = N'dbo'
	,@level1type = N'TABLE'
	,@level1name = N'calculation_table_expense_payroll_taxes';
...
summary:	>
			...
			each record describes a payroll tax rate.
Revisions:
- version 1:
		Modification: Initial script for GitHub
		Author: Zach Mueller
